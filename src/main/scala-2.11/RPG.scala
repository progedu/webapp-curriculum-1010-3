...
// bodypowerPoint=>体力 学習・変数の認知力向上のための変更
abstract class Creature(var bodypowerPoint: Int, var attackDamage: Int) {
  def isAlive(): Boolean = this.bodypowerPoint > 0
}

class Hero(_bodypowerPoint: Int, _attackDamage: Int) extends Creature(_bodypowerPoint, _attackDamage) {

  def attack(monster: Monster): Unit = {
    monster.bodypowerPoint = monster.bodypowerPoint - monster.attackDamage
  }

  def defend(monster: Monster): Unit = {
    this.bodypowerPoint = this.bodypowerPoint - (monster.attackDamage / 2)
  }

  def escape(monster: Monster): Boolean = {
    val isEscaped = RPG.random.nextInt(2) == 1
    if (!isEscaped) {
      this.bodypowerPoint = this.bodypowerPoint - monster.attackDamage
    } else {
  ...
  override def toString = s"Hero(体力:${bodypowerPoint}, 攻撃力:${attackDamage})"

}

class Monster(_hitPoint: Int, _attackDamage: Int, var isAwayFromHero: Boolean)
  extends Creature(_hitPoint, _attackDamage) {
  override def toString = s"Monster(体力:${bodypowerPoint}, 攻撃力: ${attackDamage}, ヒーローから離れている:${isAwayFromHero})"
}