...
object RPG extends App {
  ...
  while (!monsters.isEmpty) {
    ...
    if (input == "1") { // 攻撃する
      ...
      val attackDamage = hero.defend(monster)
      println(s"あなたは防御した。${attackDamage}のダメージを受けた。")
    } else { // 逃走する
...
class Hero(_hitPoint: Int, _attackDamage: Int) extends Creature(_hitPoint, _attackDamage) {
  ...
  def defend(monster: Monster): Int = {
    val attackDamage = monster.attackDamage / (RPG.random.nextInt(7) + 2)
    this.hitPoint = this.hitPoint - attackDamage
    attackDamage
  }
