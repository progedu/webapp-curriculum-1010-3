...
object RPG extends App {
  ...
  println(
    s"""あなたは冒険中の${hero}であり、
       ...
       |2を入力すると自身の攻撃力の2倍のダメージを一方的に与えることができる。
       |ただしMPが足りない場合使用できない。
       |---------------------------------------------
  ...
  while (!monsters.isEmpty) {
    ...
    } else if (input == "2") { // 魔法を唱える
      if (hero.magicPoint > 0) {
        hero.spell(monster)
        println(s"あなたは魔法を唱えた。${hero.attackDamage * 2}のダメージを与えた。")
      } else {
        println(s"あなたは魔法を唱えようとした。しかしMPが足りない。")
      }
    } else { // 逃走する
...
class Hero(_hitPoint: Int, _attackDamage: Int, var magicPoint: Int) extends Creature(_hitPoint, _attackDamage) {
  ...
  def spell(monster: Monster): Unit = {
    monster.hitPoint = monster.hitPoint - (this.attackDamage * 2)
    this.magicPoint = this.magicPoint - 50;
  }
  ...
  override def toString = s"Hero(体力:${hitPoint}, 攻撃力:${attackDamage}, MP:${magicPoint})"
